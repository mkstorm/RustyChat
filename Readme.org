...
** Autoconnect
It is also possible in the config to set servers and channels to autoconnect on command.
...
** Chat/prompt history
Use PgUp and Pgdown to scroll chat history, 500 lines are saved, though they are per instance and is not persistent.
...
- '/load'
/load
to load autojoin channels and connect (from config)
